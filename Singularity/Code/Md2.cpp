...
bool MD2File::ImportMD2(t3DModel *pModel, char *strFileName, char *strTexture)
{
	char strMessage[255] = {0};

	// Open the MD2 file in binary
	FILE *m_FilePointer = fopen(strFileName, "rb");
	...
	fread(&m_Header, 1, sizeof(MD2Header), m_FilePointer);
...
/*void MD2File::ReadMD2Data()
{
	// Create a larger buffer for the frames of animation (not fully used yet)
	unsigned char buffer[MD2_MAX_FRAMESIZE];

	// Here we allocate all of our memory from the header's information
	Skins     = new tMd2Skin [m_Header.numSkins];
	TexCoords = new tMd2TexCoord [m_Header.numTexCoords];
	Triangles = new tMd2Face [m_Header.numTriangles];
	Frames    = new tMd2Frame [m_Header.numFrames];
...
void LoadMD2::ParseAnimations(t3DModel *pModel)
{
...
void LoadMD2::ConvertDataStructures(t3DModel *pModel)
{
	int j = 0, i = 0;
...
void LoadMD2::CleanUp()
{
	// This just just the regular cleanup of our md2 model class.  We can free
